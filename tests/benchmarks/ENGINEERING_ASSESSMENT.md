...
### Q43. Page-type coverage — gap, stated honestly

The repo contains **no PDF fixtures** (verified by filesystem search); all
benchmarks use in-browser synthetic dark/light/mixed slides. Text-heavy,
image-heavy, scanned-note, and 100+ page real PDFs are **not testable
today**. Plan: generate fixtures from the app's own
`samplePdfGenerator.ts` (vector text + images + mixed pages) into
`tests/fixtures/` and extend the golden suite to them before the 1-channel
change lands.
