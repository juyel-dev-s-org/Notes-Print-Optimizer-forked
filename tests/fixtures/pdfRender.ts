...
export async function loadPdfDocument(pdfBytes: Uint8Array): Promise<{ numPages: number; destroy: () => Promise<void> }> {
  const doc = await openDoc(pdfBytes);
  return { numPages: doc.numPages, destroy: () => doc.destroy() };
}
