...
export const WorkflowUI: React.FC<WorkflowUIProps> = (props) => {
  const {
    currentPhase,
    setCurrentPhase,
    isProcessing,
    ...
    <div className="flex flex-col gap-6 pb-12 w-full max-w-full">
        ...
        <div className="flex flex-col gap-5 animate-in fade-in duration-200">
          ...
          {uploadedItems.length > 0 && (
            <div className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-xl">
              ...
              {/* Desktop File List */}
              <div className="flex flex-col gap-2 max-h-[320px] overflow-y-auto p-1 scrollbar-thin">
                {uploadedItems.map((item, idx) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-950/60 p-3 hover:bg-slate-800/50 transition-colors"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-600/30 text-indigo-300 font-bold text-xs border border-indigo-500/30">
                        {idx + 1}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-bold text-slate-100 truncate">{item.name}</p>
                        <p className="text-[11px] text-slate-400">{item.sizeMB} MB • PDF Document</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        disabled={idx === 0}
                        className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white disabled:opacity-30"
                        title="Move Up"
                      >
                      ...
                      <button
                        ...
                        className="flex h-9 w-9 items-center justify-center rounded-lg text-red-400 hover:bg-red-950/60 hover:text-red-300"
                        title="Remove File"
                      >
      ...
      {currentPhase === 4 && (
        <div className="flex flex-col items-center gap-5 text-center animate-in fade-in duration-200 max-w-xl mx-auto">
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-xl w-full text-left">
            <h3 className="text-sm font-bold text-white border-b border-slate-800 pb-2">Rate Your Experience</h3>
            {!feedbackSubmitted ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star className={`h-7 w-7 ${star <= rating ? 'fill-amber-400 text-amber-400' : 'text-slate-700'}`} />
                    </button>
                  ))}
                </div>
                <textarea
                  ...
                  placeholder="Optional feedback or suggestion..."
                  className="w-full rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs text-slate-200"
                />
                <button
                  type="button"
                  onClick={onSendFeedback}
                  className="flex h-11 justify-center items-center rounded-xl bg-slate-800 px-5 text-xs font-bold text-white hover:bg-slate-700"
                >
                  Send Feedback
                </button>
              </div>
            ) : (
              <div className="rounded-xl bg-emerald-950/60 p-4 border border-emerald-500/30 text-emerald-300 text-xs font-bold text-center">
